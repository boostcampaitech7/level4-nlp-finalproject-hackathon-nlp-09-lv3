...
# # from src.modules.Pipeline import pipe_eval #임시 확인용으로 gpt api 바로 연결 
    ...
    def answering(self, prompt: str) -> str:
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "답변과 표에 대한 내용을 참고하여 시각화 코드를 잘 구성하는 프로그래머 입니다."},
...
def generate_visualization_code(query: str, table_str: str):
    """
    초기 쿼리와 markdown 형식의 테이블 데이터를 받아, 
    matplotlib를 사용해서 해당 데이터를 시각화하는 파이썬 코드를 생성한다.
    
    GPTModel을 활용하여 코드를 생성하며, 오류 보정을 위해 2~3회 반복 시도한다.
    ...
    generated_code = None
    gpt_model = GPTModel(model_name="gpt-4o")
    
    # 2~3회 반복하여 코드 보정 시도
    for attempt in range(3):
        generated_code = gpt_model.answering(prompt)
