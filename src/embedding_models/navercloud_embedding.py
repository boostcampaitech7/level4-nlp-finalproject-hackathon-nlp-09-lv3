...
#     api_key = f"Bearer {os.getenv('NAVERCLOUD_EMBEDDING_KEY')}"
#     request_id = str(uuid.uuid4())  # 동적 request_id 생성
...
#         # HTTPS 연결 및 요청
#         conn = http.client.HTTPSConnection(host)
#         conn.request('POST', '/serviceapp/v1/api-tools/embedding/v2', 
#                      json.dumps(completion_request), headers)
#         response = conn.getresponse()
#         result = json.loads(response.read().decode(encoding='utf-8'))
#         conn.close()
...
#             return result['result']['embedding']
#         else:
#             return f"Error: {result.get('status', {}).get('message', 'Unknown error occurred')}"
